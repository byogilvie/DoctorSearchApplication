...
function doctor(practices, profile, specialties, insurances, uid) {
    ...
    this.practices = [];
    Array.prototype.push.apply(this.practices, practices);
    this.practices = this.practices.filter(function (p) {
        return p.within_search_area;
    });
...
function createForm(elements)
{
    ...
    if (elements['practName'].value != '')
    {
        ...
        processEls.insurance_uid = insuranceText;
        if (s != '')
            processEls.sort = s;
    } else
    {
...
async function processForm(formDetails, page) {
                ...
                data.data.forEach(function (item) {
                    item.doctors.forEach(function (d)
                    {
                        var doc = new doctor(item, d.profile,
                                d.specialties, item.insurance_uids, d.uid);
                        doctors.push(doc);
                    });
                });
            } else
            {
                data.data.forEach(function (item) {
                    var doc = new doctor(item.practices, item.profile,
                            item.specialties, item.insurances, item.uid);
                    doctors.push(doc);
                });
            }
            /*
             * doctors = doctors.filter(function(item) { return
                    ...
                    return new Promise(function (resolve, reject) {
                        ...
                        geocoder.geocode({
                            ...
                            if (status === google.maps.GeocoderStatus.OK) {
                                ...
                                window.alert('Geocoder failed due to: ' + status + ' at ' + delay);
                            }
                        });
                    });
                })).then(function (hours) {
